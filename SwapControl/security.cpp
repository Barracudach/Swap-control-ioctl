...
//#define DBG(format, ...) DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, format, __VA_ARGS__)
...
PVOID GetKernelBase(OUT PULONG pSize)
{
	...
	if (bytes == 0)
	{
		DBG("Invalid SystemModuleInformation size");
		return NULL;
	}
	...
	DBG("g_KernelBase: %x", g_KernelBase);
	DBG("g_KernelSize: %x", g_KernelSize);
	return g_KernelBase;
}
...
extern "C" bool LocatePiDDB(PERESOURCE * lock, PRTL_AVL_TABLE * table)
{
	...
	if (!NT_SUCCESS(BBScanSection("PAGE", PiDDBLockPtr_sig, 0, sizeof(PiDDBLockPtr_sig) - 1, reinterpret_cast<PVOID*>(&PiDDBLockPtr)))) {
		DBG("Unable to find PiDDBLockPtr sig.");
		return false;
	}
	...
	if (!NT_SUCCESS(BBScanSection("PAGE", PiDDBCacheTablePtr_sig, 0, sizeof(PiDDBCacheTablePtr_sig) - 1, reinterpret_cast<PVOID*>(&PiDDBCacheTablePtr)))) {
		DBG("Unable to find PiDDBCacheTablePtr sig");
		return false;
	}
...
void clearCache(UNICODE_STRING DriverName, ULONG timeDateStamp) {
	...
	{
		DBG("ClearCache Failed");
		return;
	}

	DBG("Found PiDDBLock and PiDDBCacheTable");
	DBG("Found PiDDBLock %x", PiDDBLock);
	DBG("Found PiDDBCacheTable %x", PiDDBCacheTable);
	...
	if (pFoundEntry == nullptr)
	{
		...
		DBG("ClearCache Failed (Not found)");
		return;
	}
...
void EraseHeader()
{
	...
	for(BYTE* ptr= (BYTE*)EraseHeader; ;ptr--)
	{
		if (bDataCompare(ptr, (BYTE*)"\x4D\x5A\x90\x00\x03\x00", "xxxxxx"))
		{
			image_base = (PVOID*)ptr;
			break;
		}
	}
	DBG(0, 0, "image_base: %I64x", image_base);
	...
	PIMAGE_DOS_HEADER pDoH = (PIMAGE_DOS_HEADER)(image_base);
	PIMAGE_NT_HEADERS pNtH = (PIMAGE_NT_HEADERS)((LONG)image_base + ((PIMAGE_DOS_HEADER)image_base)->e_lfanew);

	size = sizeof(IMAGE_DOS_HEADER);
	if (_MemoryHelper.virtual_protect(pDoH, size, PAGE_READWRITE))
		for (DWORD i = 0; i < size; i++)
			*(BYTE*)((BYTE*)pDoH + i) = 1;

	size = sizeof(IMAGE_NT_HEADERS);
	if (pNtH && _MemoryHelper.virtual_protect(pNtH, size, PAGE_READWRITE))
		for (DWORD i = 0; i < size; i++)
			*(BYTE*)((BYTE*)pNtH + i) = 1;
*/
	return;
...
NTSTATUS FindMmDriverData(VOID)
 {
	 ...
	 if (!g_KernelBase || !g_KernelSize) return STATUS_NOT_FOUND;
	 //DbgPrintEx(0, 0, "g_KernelBase: %I64x", g_KernelBase);
	// DbgPrintEx(0, 0, "g_KernelSize: %I64x", g_KernelSize);
	// return STATUS_SUCCESS;
	 PVOID MmUnloadedDriversInstr = (PVOID)FindPattern((UINT64)g_KernelBase, (UINT64)g_KernelSize,
	 ...
	 */
	 ...
	 {
		 DBG("MmUnloadedDriversInstr: %I64x", MmUnloadedDriversInstr);
		 DBG("MiRememberUnloadedDriver: %I64x", MmLastUnloadedDriverInstr);
		 return STATUS_NOT_FOUND;
	 }
	 ...
	 DBG("MmUnloadedDrivers: %I64x", MmUnloadedDrivers);
	 DBG("MmLastUnloadedDriver: %I64x", MmLastUnloadedDriver);
	 /*DBG("MmUnloadedDrivers ModuleEnd: %x", MmUnloadedDrivers->ModuleEnd);
	 DBG("MmUnloadedDrivers ModuleStart: %x", MmUnloadedDrivers->ModuleStart);
	 DBG("MmUnloadedDrivers Name: %s", MmUnloadedDrivers->Name);
	 DBG("MmUnloadedDrivers UnloadTime: %x", MmUnloadedDrivers->UnloadTime);*/

	 //DBG("MmUnloadedDrivers Addr: %x", MmUnloadedDrivers);
	 //DBG("MmLastUnloadedDriver Addr: %x", MmLastUnloadedDriver);
	 return STATUS_SUCCESS;
 }
